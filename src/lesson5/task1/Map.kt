...
fun buildGrades(grades: Map<String, Int>): Map<Int, List<String>> {
    ...
    for ((name, grade) in grades) {
        res[grade] = res[grade]?.plus(name) ?: listOf(name)
    }
    return res.toMap()
}
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    ...
    for (i in list.indices) {
        ...
        if (difference in map) {
            val res = map[number - list[i]]!!
            return min(i, res) to max(i, res)
        } else map[list[i]] = i
    }
    return -1 to -1
}
...
fun bagPacking(treasures: Map<String, Pair<Int, Int>>, capacity: Int): Set<String> {
    ...
    for (i in 1..treasures.size)
        for (j in 0..capacity)
            if (weights[i - 1] > j) backpack[i][j] = backpack[i - 1][j]
            else backpack[i][j] = max(
                backpack[i - 1][j], prices[i - 1] + backpack[i - 1][j - weights[i - 1]]
            )
    var temp = capacity
    for (i in treasures.size downTo 1) {
        if (backpack[i][temp] != backpack[i - 1][temp]) {
            ans.add(names[i - 1])
            temp -= weights[i - 1]
        }
    }
